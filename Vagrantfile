...
#Instructions for starting the environment, by environment type
	if (is_aws)
		...
		config.vm.post_up_message = "Reference environment setup complete for Docker\nRun \'docker exec reference_environment_template /bin/bash scripts run_experiments_supplementarysoftware2.sh\'\nand\n 'docker exec reference_environment_template /bin/bash scripts run_experiments_supplementarysoftware3.sh\'\nto execute the command scripts in the reference environment\n"
	else
